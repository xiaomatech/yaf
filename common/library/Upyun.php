...
class UpYun
{
    ...
    {
        $config = Yaconf::get("common")['upyun'];

        $this->_bucketname = is_null($bucketname) ? $config['bucketname'] : $bucketname;
        $this->_username = $config['username'];
        $this->_password = md5($config['password']);
        $this->_timeout = $config['timeout'];

        $this->endpoint = is_null($config['endpoint']) ? self::ED_AUTO : $config['endpoint'];
    }
