...
class AnalyticsIllegalDataException extends AnalyticsException
{
}

// 在因为网络或者不可预知的问题导致数据无法发送时，SDK会抛出此异常，用户应当捕获并处理。
class AnalyticsNetworkException extends AnalyticsException
{
}

// 当且仅当DEBUG模式中，任何网络错误、数据异常等都会抛出此异常，用户可不捕获，用于测试SDK接入正确性
class AnalyticsDebugException extends \Exception
{
}

// 不支持 Windows，因为 Windows 版本的 PHP 都不支持 long
if (strtoupper(substr(PHP_OS, 0, 3)) == "WIN") {
    throw new AnalyticsException(" Analytics PHP SDK dons't not support Windows");
}
...
class Analytics
{
    ...
    private function _normalize_data($data)
    {
        ...
        if (!isset($data['distinct_id']) or strlen($data['distinct_id']) == 0) {
            throw new AnalyticsIllegalDataException("property [distinct_id] must not be empty");
        }
        if (strlen($data['distinct_id']) > 255) {
            throw new AnalyticsIllegalDataException("the max length of [distinct_id] is 255");
        }
        ...
        if ($ts_num < 10 || $ts_num > 13) {
            throw new AnalyticsIllegalDataException("property [time] must be a timestamp in microseconds");
        }
        ...
        if (isset($data['event']) && !preg_match($name_pattern, $data['event'])) {
            throw new AnalyticsIllegalDataException("event name must be a valid variable name. [name='${data['event']}']");
        }
        ...
        if (isset($data['properties']) && is_array($data['properties'])) {
            foreach ($data['properties'] as $key => $value) {
                if (!is_string($key)) {
                    throw new AnalyticsIllegalDataException("property key must be a str. [key=$key]");
                }
                if (strlen($data['distinct_id']) > 255) {
                    throw new AnalyticsIllegalDataException("the max length of property key is 256. [key=$key]");
                }

                if (!preg_match($name_pattern, $key)) {
                    throw new AnalyticsIllegalDataException("property key must be a valid variable name. [key='$key']]");
                }

                // 只支持简单类型或数组或DateTime类
                if (!is_scalar($value) && !is_array($value) && !$value instanceof DateTime) {
                    throw new AnalyticsIllegalDataException("property value must be a str/int/float/datetime/list. [key='$key' value='$value']");
                }
                ...
                if (is_string($value) && strlen($data['distinct_id']) > 8191) {
                    throw new AnalyticsIllegalDataException("the max length of property value is 8191. [key=$key]");
                }

                // 如果是数组，只支持 Value 是字符串格式的简单非关联数组
                if (is_array($value)) {
                    if (array_values($value) !== $value) {
                        throw new AnalyticsIllegalDataException("[list] property must not be associative. [key='$key']");
                    }

                    foreach ($value as $lvalue) {
                        if (!is_string($lvalue)) {
                            throw new AnalyticsIllegalDataException("[list] property's value must be a str. [value='$lvalue']");
                        }
                    }
                }
            }
            ...
            throw new AnalyticsIllegalDataException("property must be an array.");
        }
        return $data;
    }
    ...
    public function track_signup($distinct_id, $original_id, $properties = array())
    {
        ...
        if (!$original_id or strlen($original_id) == 0) {
            throw new AnalyticsIllegalDataException("property [original_id] must not be empty");
        }
        if (strlen($original_id) > 255) {
            throw new AnalyticsIllegalDataException("the max length of [original_id] is 255");
        }
...
class DebugConsumer extends AbstractConsumer
{
    ...
    public function send($msg)
    {
        ...
        if ($response['ret_code'] >= 300) {
            throw new AnalyticsDebugException("Unexpected response from Analytics.");
        }
    }
    ...
    protected function _do_request($data)
    {
        ...
        if (!$http_response_header) {
            throw new AnalyticsDebugException(
                "Failed to connect to Analytics. [error='" + curl_error($ch) + "']");
        }
