...
class db_Db
{
    ...
    {
        $config_db = Yaconf::get('common')['database'];
        if (!empty($db_config) && is_string($db_config)) {
            ...
            $db_config = array(
                ...
                'dsn' => isset($db_config['dsn']) ? $db_config['dsn'] : '',
                'params' => isset($db_config['params']) ? $db_config['params'] : array(),
                'charset' => isset($db_config['charset']) ? $db_config['charset'] : 'utf8',
            );
        } elseif (empty($db_config)) {
            // 如果配置为空，读取配置文件设置
            if ($config_db['dsn'] && 'pdo' != strtolower($config_db['type'])) { // 如果设置了DB_DSN 则优先
                $db_config = $this->parseDSN($config_db['dsn']);
            } else {
                $db_config = array(
                    'dbms' => $config_db['type'],
                    'username' => $config_db['user'],
                    'password' => $config_db['pwd'],
                    'hostname' => $config_db['host'],
                    'hostport' => $config_db['port'],
                    'database' => $config_db['name'],
                    'dsn' => $config_db['dsn'],
                    'params' => $config_db['params'],
    ...
    protected function multiConnect($master = false)
    {
        ...
        $config_db = Yaconf::get('common')['database'];
        // 数据库读写是否分离
        if ($config_db['rw_separate']) {
            // 主从式采用读写分离
            if ($master)
                // 主服务器写入
                $r = floor(mt_rand(0, $config_db['master_num'] - 1));
            else {
                if (is_numeric($config_db['slave_no'])) {// 指定服务器读
                    $r = $config_db['slave_no'];
                } else {
                    // 读操作连接从服务器
                    $r = floor(mt_rand($config_db['master_num'], count($_config['hostname']) - 1));   // 每次随机连接的数据库
                }
            }
        } else {
            // 读写操作不区分服务器
    ...
    protected function parseSet($data)
    {
        ...
        foreach ($data as $key => $val) {
            if (is_array($val) && 'exp' == $val[0]) {
                ...
                if ($config_db['bind_param'] && 0 !== strpos($val, ':')) {
                    $name = md5($key);
    ...
    protected function parseWhereItem($key, $val)
    {
        ...
        if (is_array($val)) {
            ...
            $config_db = Yaconf::get('common')['database'];
            //对字符串类型字段采用模糊匹配
            if ($config_db['like_fields'] && preg_match('/^(' . $config_db['like_fields'] . ')$/i', $key)) {
                $val = '%' . $val . '%';
