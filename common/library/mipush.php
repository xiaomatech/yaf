...
class Mipush
{
    ...
    public function __construct($os_type = '', $config = array(), $options = array(), $environment = '')
    {
        ...
        $mipush_config = Yaconf::get('common')['mipush'];

        /* init option */
        $this->_headers = [];
        $this->_headers[] = 'Authorization: key=' . $mipush_config['secret'];
        if ($os_type === 'android') {
            $this->_options['restricted_package_name'] = $mipush_config['package_name'];
        }
