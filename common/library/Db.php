...
class Db
{
    ...
    public static function connect($config = '_')
    {
        if (is_array($config)) {
            ...
            $key = $config;
            $config = Yaconf::get("common")['database']['db.' . $key];
        ...
        if (Yaconf::get("common")['database']['exception']) {
    ...
    public static function get($name = '_')
    {
        ...
        return Db::$_dbpool[$name] = Yaconf::get("common")['database']['db.' . $name . '.dsn'] ? Db::connect($name) : Db::connect('_');
    }
