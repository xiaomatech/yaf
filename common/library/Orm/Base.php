...
class Orm_Base
{
    ...
    function __construct($pConfig = 'default')
    {
        ...
        $tDB = Yaconf::get('common')['db'][$pConfig];
    ...
    function fRow($pId = 0)
    {
        if (false === stripos($pId, 'SELECT')) {
            ...
            $tSql = "SELECT " . $topone . " {$tOpt['field']} FROM `{$tOpt['table']}` {$tOpt['where']} {$tOpt['order']}  " . $limitCondition;
        } else {
            $tSql = &$pId;
        }
            ...
            if (is_file($tFile)) {
                ...
                if ($this->_dbtype == 'mysql') {
                    ...
                    if ($tQuery = $this->db->query("SHOW FULL FIELDS FROM `$table`")) {
                        ...
                        if (!is_dir(dirname($tFile))) {
                            mkdir(dirname($tFile), 0755, true);
                        }
